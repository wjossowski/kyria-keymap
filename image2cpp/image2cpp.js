...
const TODOBuildConversionFunctions = require("./ConversionFunctions");
...
const fn = async (argv) => {
        ...
        static vertical1bit(data, canvasWidth, canvasHeight) {
            ...
            for (let p = 0; p < Math.ceil(settings.screenHeight / 8); p++) {
                for (let x = 0; x < settings.screenWidth; x++) {
                    ...
                    for (let y = 7; y >= 0; y--) {
                        let index =
                            (p * 8 + y) * (settings.screenWidth * 4) + x * 4;
                        let avg =
    ...
    const settings = {
        ...
        conversionFunction:
            ConversionFunctions[argv.mode] ||
            ConversionFunctions.horizontal1bit,
    };
    ...
    async function handleImageSelection(input) {
        ...
        console.log(
            `canvas size: ${settings.screenWidth} x ${settings.screenHeight}`
        );
    ...
    function getType() {
        if (settings.conversionFunction == ConversionFunctions.horizontal565) {
            ...
            settings.conversionFunction == ConversionFunctions.horizontal888
        ) {
